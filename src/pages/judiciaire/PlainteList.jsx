...
export default function PlainteList() {
  const [data, setData] = useState([]);
  const token = localStorage.getItem("token");

  // Récupération du user & rôle admin
  const user = JSON.parse(localStorage.getItem("user") || "{}");
  const isAdmin = Array.isArray(user.roles) && user.roles.includes("admin");

  useEffect(() => {
    axios
      .get("http://localhost:8000/api/activities/judiciaire/plainte/", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/activities/judiciaire/plainte/export/",
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
      ...
      link.href = url;
      link.setAttribute("download", "plaintes.xlsx");
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-4">Plaintes</h2>
      <div className="mb-4 flex gap-2">
        <Link
          to="/judiciaire/plainte/add"
            ...
            {Array.isArray(data) && data.length > 0 ? (
              data.map((obj) => (
                <tr key={obj.id} className="hover:bg-gray-100 transition rounded-xl">
                  <td className="px-6 py-2">{obj.date_plainte}</td>
                  <td className="px-4 py-2 text-center">{obj.nombre_plainte}</td>
                  <td className="px-4 py-2 text-center flex justify-center gap-2">
                    <Link
                      to={`/judiciaire/plainte/${obj.id}/edit`}
