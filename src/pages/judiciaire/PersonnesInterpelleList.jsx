...
export default function PersonnesInterpelleList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/activities/judiciaire/interpellations/", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/activities/judiciaire/interpellations/export/",
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
      ...
      link.href = url;
      link.setAttribute("download", "personnes_interpellees.xlsx");
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-4">Personnes interpellées</h2>
      <div className="mb-4 flex gap-2">
        <Link
          to="/judiciaire/personnes-interpelle/add"
            ...
            {Array.isArray(data) && data.length > 0 ? (
              data.map((obj) => (
                <tr key={obj.id} className="hover:bg-gray-100 transition rounded-xl">
                  ...
                  <td className="px-4 py-2 text-center">{obj.nombre_pers_interp}</td>
                  <td className="px-4 py-2 text-center flex justify-center gap-2">
                    <Link
                      to={`/judiciaire/personnes-interpelle/${obj.id}/edit`}
