...
export default function DeclarationVolList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/activities/judiciaire/declaration-vols/", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/activities/judiciaire/declaration-vols/export/",
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
      ...
      link.href = url;
      link.setAttribute("download", "declaration_vols.xlsx");
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-4">Déclarations de vol</h2>
      <div className="mb-4 flex gap-2">
        <Link
          to="/judiciaire/declaration-vol/add"
            ...
            {Array.isArray(data) && data.length > 0 ? (
              data.map((obj) => (
                <tr key={obj.id} className="hover:bg-gray-100 transition rounded-xl">
                  <td className="px-4 py-2">{obj.date_plainte}</td>
                  <td className="px-4 py-2">{obj.type_vol}</td>
                  <td className="px-4 py-2 text-center">{obj.nombre_vol}</td>
                  <td className="px-4 py-2">{obj.quartier || <span className="text-gray-400">—</span>}</td>
                  <td className="px-4 py-2 text-center flex justify-center gap-2">
                    <Link
                      to={`/judiciaire/declaration-vol/${obj.id}/edit`}
