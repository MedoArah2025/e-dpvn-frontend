...
export default function VehiculeEnleveList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/activities/judiciaire/vehicules-enleves/", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/activities/judiciaire/vehicules-enleves/export/",
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
      ...
      link.href = url;
      link.setAttribute("download", "vehicules_enleves.xlsx");
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-4">Véhicules enlevés</h2>
      <div className="mb-4 flex gap-2">
        <Link
          to="/judiciaire/vehicule-enleve/add"
            ...
            {Array.isArray(data) && data.length > 0 ? (
              data.map((obj) => (
                <tr key={obj.id} className="hover:bg-gray-100 transition rounded-xl">
                  ...
                  <td className="px-4 py-2 text-center">{obj.nbr_enleves}</td>
                  <td className="px-4 py-2 text-center flex justify-center gap-2">
                    <Link
                      to={`/judiciaire/vehicule-enleve/${obj.id}/edit`}
