...
export default function AutreSaisieList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/activities/judiciaire/autres-saisies/", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/activities/judiciaire/autres-saisies/export/",
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
      ...
      link.href = url;
      link.setAttribute("download", "autres_saisies.xlsx");
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-4">Autres saisies</h2>
      <div className="mb-4 flex gap-2">
        <Link
          to="/judiciaire/autre-saisie/add"
              ...
              data.map((obj) => (
                <tr key={obj.id} className="hover:bg-gray-100 transition rounded-xl">
                  ...
                  <td className="px-4 py-2 text-center flex justify-center gap-2">
                    <Link
                      to={`/judiciaire/autre-saisie/${obj.id}/edit`}
