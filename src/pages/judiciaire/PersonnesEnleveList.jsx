...
export default function PersonnesEnleveList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/activities/judiciaire/enlevements/", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/activities/judiciaire/enlevements/export/",
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
      ...
      link.href = url;
      link.setAttribute("download", "personnes_enlevees.xlsx");
  ...
  // Suppression sécurisée pour admin
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-4">Personnes enlevées</h2>
      <div className="mb-4 flex gap-2">
        <Link
          to="/judiciaire/personnes-enleve/add"
            ...
            {Array.isArray(data) && data.length > 0 ? (
              data.map((obj) => (
                <tr key={obj.id} className="hover:bg-gray-100 transition rounded-xl">
                  ...
                  <td className="px-4 py-2 text-center">{obj.nbr_enlevees}</td>
                  <td className="px-4 py-2 text-center flex justify-center gap-2">
                    <Link
                      to={`/judiciaire/personnes-enleve/${obj.id}/edit`}
