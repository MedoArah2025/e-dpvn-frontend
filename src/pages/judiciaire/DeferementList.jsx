...
export default function DeferementList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/activities/judiciaire/deferements/", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/activities/judiciaire/deferements/export/",
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
      ...
      link.href = url;
      link.setAttribute("download", "deferements.xlsx");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8000/api/activities/judiciaire/deferements/${id}/`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-4">Déférements</h2>
      <div className="mb-4 flex gap-2">
        <Link
          to="/judiciaire/deferement/add"
            ...
            {Array.isArray(data) && data.length > 0 ? (
              data.map((obj) => (
                <tr key={obj.id} className="hover:bg-gray-100 transition rounded-xl">
                  <td className="px-6 py-2">{obj.date_interpellation}</td>
                  <td className="px-4 py-2">{obj.categorie_infraction}</td>
                  <td className="px-4 py-2 text-center">{obj.nombre_deferement}</td>
                  <td className="px-4 py-2 text-center flex justify-center gap-2">
                    <Link
                      to={`/judiciaire/deferement/${obj.id}/edit`}
