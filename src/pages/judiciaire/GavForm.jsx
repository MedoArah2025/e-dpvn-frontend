...
export default function DeferementForm() {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`http://localhost:8000/api/activities/judiciaire/deferements/${id}/`, {
  ...
  function handleSubmit(e) {
    ...
    const url = id
      ...
      : `http://localhost:8000/api/activities/judiciaire/deferements/`;
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-lg mx-auto bg-white p-8 rounded shadow">
      <h2 className="text-xl font-bold mb-4">{id ? "Modifier" : "Ajouter"} un Défèrement</h2>
      ...
      <label className="block mb-2 font-semibold">
        ...
        <input
          type="number"
          name="nombre_deferement"
          className="block w-full border rounded px-3 py-2"
          value={form.nombre_deferement}
          onChange={handleChange}
          min="0"
          required
        />
      ...
      <div className="mt-4 flex gap-2">
        ...
        <button
          type="button"
          onClick={() => navigate("/judiciaire/deferement")}
