...
export default function DeclarationPerteList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/activities/administratif/declarations-perte/", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/activities/administratif/declarations-perte/export/",
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
      ...
      link.href = url;
      link.setAttribute("download", "declarations_perte.xlsx");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8000/api/activities/administratif/declarations-perte/${id}/`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-4">Déclarations de perte</h2>
      <div className="mb-4 flex gap-2">
        <Link
          to="/administratif/declaration-perte/add"
              ...
              data.map((obj) => (
                <tr key={obj.id} className="hover:bg-gray-100 transition rounded-xl">
                  ...
                  <td className="px-4 py-2 text-center flex justify-center gap-2">
                    <Link
                      to={`/administratif/declaration-perte/${obj.id}/edit`}
