...
export default function CinList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/activities/administratif/cin/", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/activities/administratif/cin/export/",
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
      ...
      link.href = url;
      link.setAttribute("download", "cin.xlsx");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8000/api/activities/administratif/cin/${id}/`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-4">Cartes d'identité nationales (CIN)</h2>
      <div className="mb-4 flex gap-2">
        <Link
          to="/administratif/cin/add"
            ...
            {Array.isArray(data) && data.length > 0 ? (
              data.map((obj) => (
                <tr key={obj.id} className="hover:bg-gray-100 transition rounded-xl">
                  ...
                  <td className="px-4 py-2">{obj.carte_etablie}</td>
                  <td className="px-4 py-2">{obj.carte_reprise}</td>
                  <td className="px-4 py-2">{obj.duplicata}</td>
                  <td className="px-4 py-2 text-center flex justify-center gap-2">
                    <Link
                      to={`/administratif/cin/${obj.id}/edit`}
