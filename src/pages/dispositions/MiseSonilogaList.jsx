...
export default function MiseSonilogaList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/activities/dispositions/soniloga/", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/activities/dispositions/soniloga/export/",
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
      ...
      link.href = url;
      link.setAttribute("download", "mises_soniloga.xlsx");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8000/api/activities/dispositions/soniloga/${id}/`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-4">Mises à disposition Soniloga</h2>
      <div className="mb-4 flex gap-2">
        <Link
          to="/dispositions/soniloga/add"
              ...
              data.map((obj) => (
                <tr key={obj.id} className="hover:bg-gray-100 transition rounded-xl">
                  ...
                  <td className="px-4 py-2 text-center flex justify-center gap-2">
                    <Link
                      to={`/dispositions/soniloga/${obj.id}/edit`}
