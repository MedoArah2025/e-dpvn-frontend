...
export default function RaffleList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/activities/operations/raffles/", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/activities/operations/raffles/export/",
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
      ...
      link.href = url;
      link.setAttribute("download", "raffles.xlsx");
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Confirmer la suppression de cette raffle ?")) return;
    try {
      await axios.delete(
        `http://localhost:8000/api/activities/operations/raffles/${id}/`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-4 flex gap-2">
        <Link to="/operation/raffle/add" className={addBtn}>
          + Ajouter
        </Link>
        ...
        <table className="min-w-full table-auto">
          <thead>
            <tr>
              ...
              <th className={thClass}>Personnes interpellées</th>
              <th className={thClass}>Mendiants interpellés</th>
              <th className={thClass}>Charrettes/pouspous</th>
              <th className={thClass}>Objet saisi</th>
            ...
            {Array.isArray(data) && data.length > 0 ? (
              data.map((obj) => (
                <tr key={obj.id} className="hover:bg-gray-100 transition">
                  ...
                  <td className={cellClass}>{obj.personne_interpellees}</td>
                  <td className={cellClass}>{obj.mendiants_interpelles}</td>
                  <td className={cellClass}>{obj.charettes_pouspous}</td>
                  <td className={cellClass}>{obj.objet_saisie}</td>
                  <td className={cellClass + " flex items-center"}>
                    <Link
                      to={`/operation/raffle/${obj.id}/edit`}
