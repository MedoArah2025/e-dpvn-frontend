...
export default function CoupPoingForm() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`http://localhost:8000/api/activities/operations/coups-poing/${id}/`, {
  ...
  function handleChange(e) {
    ...
    setForm((f) => ({
      ...f,
      [name]: name.includes("nombre") || name.endsWith("_interpellees") || name.endsWith("_pouspous")
        ? parseInt(value, 10)
        : value,
    }));
  }
  ...
  function handleSubmit(e) {
    ...
    const url = id
      ...
      : `http://localhost:8000/api/activities/operations/coups-poing/`;
    axios[method](url, form, {
      headers: { Authorization: `Bearer ${token}` },
    }).then(() => navigate("/operation/coup-poing"));
  }

  return (
    <form onSubmit={handleSubmit} className="max-w-lg mx-auto bg-white p-8 rounded shadow">
      <h2 className="text-xl font-bold mb-4">{id ? "Modifier" : "Ajouter"} Coup de poing</h2>
      ...
      <div className="mt-4 flex gap-2">
        ...
        <button
          type="button"
          onClick={() => navigate("/operation/coup-poing")}
